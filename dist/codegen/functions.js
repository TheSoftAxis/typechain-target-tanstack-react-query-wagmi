...
function generateFunction(options, fn, contractName, overloadedName) {
    let inputTypes = (0, types_1.generateInputTypes)(fn.inputs, {})
    ...
    ${`use${(0, typechain_1.normalizeName)(fn.name)}`} = (${inputTypes}) => {
      const provider = useProvider(this.networkId);
      const contract = ${contractName}__factory.connect(this.contractAddress, provider);
      return useQuery<${(0, types_1.generateOutputTypes)({}, fn.outputs)}>(["${fn.name}", "${contractName}", ${inputNames}], async () => {
        return await contract.${fn.name}( ${inputNames}, overrides);
      });
    };
  `;
    }
